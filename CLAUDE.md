...
Unity 6 项目（URP 17.5.0）：**AI 关卡生成器** —— 通过 AI（MCP + LLM）在 Unity 编辑器中自动生成关卡的工具项目。当前处于**接口/数据层 + 资源映射已实现、生成链路未接入**的阶段：

- 已实现：数据 DTO（`GenerationRequest/Result`、`LevelData`、`TaskData`）、核心接口（`IGenerator`、`IResourceMapper`、`ITemplateProvider`、`IValidator<T>`、`ILogger`）、模板基类（`LevelTemplate`/`TaskTemplate`）、校验基类（`BaseValidator<T>`）、编辑器主窗口（UI Toolkit）、**资源映射系统**（`PrefabMappingConfig` + `ResourceMappingManager`，含精确/别名/模糊匹配，见下"资源映射系统"）、EditMode 单元测试（10 个，全部通过）。
- 未实现（`Assets/Editor/AILevelGenerator/{Builders,Core,Utils}`、`Assets/Editor/CLI/`、`Assets/RunTime/AILevelGenerator/{Entities,Tasks,Utilities}`、`Assets/GlobalSettings/*`、`Assets/Settings/{GeneratorConfig,Templates}` 均为空占位）：LLM 调用、生成调度器、校验器实现、模板配置资产。窗口内注释标注了 "Day5" 计划接入点。

### 程序集（asmdef）结构

- **`AILevelGenerator.Runtime`**（`Assets/RunTime/AILevelGenerator/` 下的 asmdef）：全部运行时模块代码（Data/Interfaces/Validation/Mappings）。`Assets/RunTime/PlayerMovement.cs` 在 asmdef 目录之外，仍属 `Assembly-CSharp`。所有新建 Runtime 代码都应放入该程序集。
...
- **命名空间是 `AILevelGenerator.Runtime`**（注意：目录名是 `RunTime`，代码里是 `Runtime`，两个拼写都存在于仓库中，写新代码时用后者）。
...
```
GenerationRequest (Prompt/TemplateId/RandomSeed/开关)
   → IGenerator.GenerateAsync()          // 屏蔽具体 LLM 实现
   → GenerationResult                    // LevelData + List<TaskData> + ValidationError/Warning 列表
   → IValidator<T>.Validate(data, ValidationContext)   // 泛型校验器，Error 阻断 / Warning 仅提示
