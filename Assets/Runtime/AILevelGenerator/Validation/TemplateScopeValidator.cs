    ...
    public class TemplateScopeValidator : BaseValidator<LevelData>
    {
        ...
        public override ValidationResult Validate(LevelData data, ValidationContext context)
        {
            ...
            if (_template.MaxPropCount > 0 && propCount > _template.MaxPropCount)
                AddError(result, ErrorCodes.PROPS_TOO_MANY, $"道具数量 {propCount} 超过模板上限 {_template.MaxPropCount}", "props");
            if (_template.MinPropCount > 0 && propCount < _template.MinPropCount)
                AddError(result, ErrorCodes.PROPS_TOO_FEW, $"道具数量 {propCount} 低于模板下限 {_template.MinPropCount}", "props");

            var taskCount = data.Tasks?.Count ?? 0;
            if (_template.MaxTaskCount > 0 && taskCount > _template.MaxTaskCount)
                AddError(result, ErrorCodes.TASKS_TOO_MANY, $"任务数量 {taskCount} 超过模板上限 {_template.MaxTaskCount}", "tasks");
            if (_template.MinTaskCount > 0 && taskCount < _template.MinTaskCount)
                AddError(result, ErrorCodes.TASKS_TOO_FEW, $"任务数量 {taskCount} 低于模板下限 {_template.MinTaskCount}", "tasks");

            // 主线任务强制（与 LLM 侧同文案）
            if (_template.ForceMainTask && !HasMainTask(data.Tasks))
                AddError(result, ErrorCodes.NO_MAIN_TASK, "模板要求存在主线任务，但生成结果没有 IsMainTask=true 的任务", "tasks");
            ...
            if (_template.OverrideTerrain && data.Terrain != null)
            {
                if (data.Terrain.Width != _template.TerrainWidth)
                    AddError(result, ErrorCodes.TERRAIN_MISMATCH, $"地形宽度与模板不一致：{data.Terrain.Width} ≠ {_template.TerrainWidth}", "terrain.width");
                if (data.Terrain.Length != _template.TerrainLength)
                    AddError(result, ErrorCodes.TERRAIN_MISMATCH, $"地形长度与模板不一致：{data.Terrain.Length} ≠ {_template.TerrainLength}", "terrain.length");
                if (!Mathf.Approximately(data.Terrain.HeightScale, _template.TerrainHeightScale))
                    AddError(result, ErrorCodes.TERRAIN_MISMATCH, $"地形高度缩放与模板不一致：{data.Terrain.HeightScale} ≠ {_template.TerrainHeightScale}", "terrain.heightScale");
            }
