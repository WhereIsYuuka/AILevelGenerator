...
Unity 6 项目（URP 17.5.0）：**AI 关卡生成器** —— 通过 AI（MCP + LLM）在 Unity 编辑器中自动生成关卡的工具项目。当前处于**第五周「模板化扩展与生产优化（瘦身聚焦）」，2026-09-02 已完成 Day1（模板确定性机制）**。第五周周框架/逐日开工要点见 [Docs/第五周开发计划.md](Docs/第五周开发计划.md)（**新对话交接入口，开工先读**）；前四周交付基线：

- 已实现：数据 DTO（`GenerationRequest/Result`、`LevelData`、`TaskData`）、核心接口（`IGenerator`、`IResourceMapper`、`ITemplateProvider`、`IValidator<T>`、`ILogger`）、模板基类（`LevelTemplate`/`TaskTemplate`）、校验基类（`BaseValidator<T>`）、编辑器主窗口（UI Toolkit）、**资源映射系统**（`PrefabMappingConfig` + `ResourceMappingManager`，含精确/别名/模糊匹配，见下"资源映射系统"）、**调度控制层（Day5）**：生成任务状态机（准备/生成中/成功/失败）+ 异步调度框架（`GeneratorScheduler` + `IGeneratorScheduler` + `ServiceLocator`）+ `MockGenerator` 占位生成器 + 窗口→调度→日志链路（状态行显示、生成中按钮禁用）、**前置校验体系（第四周-Day2）**：`ValidatorRegistry` 可插拔注册表（阶段+模板+类型三重匹配调度）+ 4 个前置校验器（RequestValidator 输入合法性 / ResourceValidator 资源存在性 / DataBoundsValidator 数值边界 / TemplateScopeValidator 模板范围，每个模板注册自己的校验器）+ 调度器接入（非法输入 100% 拦截；构建失败自动全量回滚）、**生成中/后置校验体系（第四周-Day3）**：Mid 阶段（`MidValidationAccumulator` 增长前缀累积，每帧批次跑 DataBounds/Resource 校验，失败立即终止构建 + 两级回滚兜底）+ Post 阶段（`PostBuildValidator` 实体空引用 `POST_ENTITIES_MISSING/COUNT_MISMATCH/ENTITY_NULL` + 组件完整性 `POST_COMPONENT_MISSING` + 逻辑可达性 `POST_FLOAT_UNSUPPORTED`，地面预探测防误报 `POST_GROUND_MISSING` 警告降级；失败转 Failed + 自动全量回滚）。EditMode 单元测试 **388 个全部通过**（原 367 + 第五周-Day1 新增 21 例）。第五周-Day1 变更：`DeterministicRandom`/`RandomSeedUtility`（确定性随机）+ 模板基类新增 `FinalizeData(data, seed)` 统一生命周期（ApplyDefaults + PostGenerate 确定性钩子），生成器一律走 FinalizeData 禁散调 ApplyDefaults；**模板随机逻辑严禁混用 UnityEngine.Random/System.Random**。
- 未实现（`Assets/Editor/AILevelGenerator/{Utils}`、`Assets/Editor/CLI/`、`Assets/Runtime/AILevelGenerator/{Entities,Tasks}`、`Assets/GlobalSettings/*`、`Assets/Settings/Templates` 等为空占位）：校验器错误码提示 UI。
...
窗口点击生成 → GeneratorScheduler.StartGenerationAsync(request)   // 经 ServiceLocator 获取，窗口不 new 业务类
   ...
   → 场景构建器分帧实例化（SceneLevelBuilder）          // 每帧批次后 Mid 生成中校验（增长前缀累积，失败立即终止）
   → 后置校验（Run Post 阶段，实体空引用/组件完整性/逻辑可达性）  // 失败 → 转 Failed + 自动全量回滚（RollbackToSnapshot + ResetToReady）
   → 构建失败/异常 → 自动全量回滚（RollbackToSnapshot + ResetToReady）
   → IValidator<T>.Validate(data, ValidationContext)   // 三层校验体系（Day2 Pre + Day3 Mid/Post 已全部接入）
```
