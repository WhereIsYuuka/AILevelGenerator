...
namespace AILevelGenerator.Runtime.Validation
{
    ...
    public class ResourceValidator : BaseValidator<LevelData>
    {
        public override ValidationResult Validate(LevelData data, ValidationContext context)
        {
            ...
            {
                AddError(result, ErrorCodes.RESOURCE_MAPPER_MISSING, "资源映射服务未注入，无法校验资源存在性", "resourceMapper");
                return result;
            }
            ...
            {
                AddError(result, ErrorCodes.RESOURCE_MAPPING_EMPTY, "资源映射表为空，请先配置 PrefabMappingConfig", "resourceMapper");
                return result;
            }
            ...
            for (var i = 0; i < data.Props.Count; i++)
            {
                ...
                {
                    AddError(result, ErrorCodes.RESOURCE_NAME_EMPTY, "道具逻辑名为空，无法映射预制体", path);
                    continue;
                }
                if (!context.ResourceMapper.TryGetPrefab(prop.PrefabLogicalName, out _))
                {
                    AddError(result, ErrorCodes.RESOURCE_NOT_FOUND, $"资源不存在：{prop.PrefabLogicalName}，请检查资源映射配置", path);
                }
            }
